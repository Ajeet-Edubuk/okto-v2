...
console.log("baseurl",OktoBaseUrl);
...
export async function executeUserOp(userop: any, authToken: string) {
  try {
    ...
    console.log("auth token in raw:",authToken);
    const response = await axios.post(
      `${OktoBaseUrl}/api/oc/v1/execute`, 
      userop,
      {
        headers: {
    ...
    console.error("Error executing user operation:", error.response);
    throw error;
  }
}

export async function getUserOperationGasPrice(authToken: string) {
  console.log("auth token in gas price:",authToken);
  const response = await axios.get(
    `${OktoBaseUrl}/api/oc/v1/gas-values`, 
    {
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
